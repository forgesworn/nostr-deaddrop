...
describe('drop key derivation', () => {
  it('is symmetric with no ephemeral', () => {
    const a = deriveDropKey({ myPrivateKey: A, peerPublicKey: pubB }, EPOCH)
    const b = deriveDropKey({ myPrivateKey: B, peerPublicKey: pubA }, EPOCH)
  ...
  it('is symmetric with both ephemerals', () => {
    const a = deriveDropKey({ myPrivateKey: A, peerPublicKey: pubB, myEphemeralPrivateKey: EA, peerEphemeralPublicKey: epubB }, EPOCH)
    const b = deriveDropKey({ myPrivateKey: B, peerPublicKey: pubA, myEphemeralPrivateKey: EB, peerEphemeralPublicKey: epubA }, EPOCH)
    expect(a.case).toBe('both')
    expect(a.publicKey).toBe(b.publicKey)
  })
  it('is symmetric one-sided, and A-carries differs from B-carries', () => {
    const aCarries1 = deriveDropKey({ myPrivateKey: A, peerPublicKey: pubB, myEphemeralPrivateKey: EA }, EPOCH)
    const aCarries2 = deriveDropKey({ myPrivateKey: B, peerPublicKey: pubA, peerEphemeralPublicKey: epubA }, EPOCH)
    expect(aCarries1.case).toBe('one')
    expect(aCarries1.publicKey).toBe(aCarries2.publicKey)
    const bCarries = deriveDropKey({ myPrivateKey: A, peerPublicKey: pubB, peerEphemeralPublicKey: epubB }, EPOCH)
    expect(bCarries.publicKey).not.toBe(aCarries1.publicKey)
  })
  it('rotates per epoch and per case', () => {
    const e0 = deriveDropKey({ myPrivateKey: A, peerPublicKey: pubB }, EPOCH)
    const e1 = deriveDropKey({ myPrivateKey: A, peerPublicKey: pubB }, EPOCH + 1)
    const both = deriveDropKey({ myPrivateKey: A, peerPublicKey: pubB, myEphemeralPrivateKey: EA, peerEphemeralPublicKey: epubB }, EPOCH)
    expect(new Set([e0.publicKey, e1.publicKey, both.publicKey]).size).toBe(3)
  })
  it('window covers previous, current and next epoch', () => {
    const w = deriveDropWindow({ myPrivateKey: A, peerPublicKey: pubB }, 1793577600)
